 ...
 * dates the sprint actually ran on. Starting one millisecond after the end keeps
 * the two windows from sharing a day.
 */
export function nextWindowAfter(
  ...
  const from = new Date(sprintEnd.getTime() + 1);
