...
- [x] 1.1 `ci.yml` valid YAML with both `test` and `integration` jobs (parse / actionlint) — 89a6ea1
- [x] 1.2 PR `integration` job green: supabase start -x → db:migrate → test:integration (4 tests) — 89a6ea1
- [x] 1.3 PR `test` job stays green — 89a6ea1
- [x] 1.4 Local sanity: `npm run test:integration` + `npm test` green, no regressions — 89a6ea1
- [x] 1.5 `dotenv` declared in `package.json` devDependencies; `npm ci` + `npm test` green — 89a6ea1
