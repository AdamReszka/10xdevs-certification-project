...
| S-17 | working-days-calendar     | public holidays are DERIVED automatically from the team's country (per-sprint team-wide days off ship in S-23, entered by hand) | S-08, S-23 | FR-007, FR-009, FR-010                          | done     |
| S-18 | next-sprint-capacity      | the availability tab forecasts the NEXT window's capacity, not just who is away                | S-08               | FR-010                                          | done     |
...
| S-32 | cadence-single-home       | the cadence a lead chose has exactly ONE home in the database — the superseded copy on `sprint` is dropped, the reader guard holding the line goes with it, and the retention question is answered: nothing prunes the record, on purpose | S-30               | FR-007                                          | done |
...
| S-17       | working-days-calendar     | Public holidays derived automatically from the team's country          | done                   | ✅ Implemented, reviewed & merged — PR #105 (2026-08-31), four phases. **Three things shipped, and the smallest of them is the derivation.** The seam was already fed (S-23), so what was missing was the country, the recurrence and the SENTENCE: no real account held a single `team_day_off` row, which means the man-day divisor and all five elapsed-time budgets had been running against an empty calendar and saying nothing about it. `holiday_year_approval` — not `team_day_off.source` — is what keeps a deleted holiday deleted: a deleted row leaves no provenance behind to consult, so only a record saying *this year has been decided about* can stop the next render re-offering it. Phase 1's "your calendar is empty" notice is deliberately RETIRED in Phase 4: the approval record, not the row count, is what says the calendar was reviewed, and a team that unchecked every holiday because it works them verified the opposite of what that sentence claimed. Migration `0025` applied to production the same day via the Supabase MCP route (backlog row 28.0). Impl-review F1/F3 fixed on the branch: the proposal window followed `getActiveSprintRow`, which returns the most-recently-STARTED sprint when none is active, so between sprints the surfaces asked about a closed sprint's year and went silent on the year the team was living in — the annual regression the two-year window exists to prevent, one level up. Eleven manual rows open in `manual-test-backlog.md` §28 |
...
| S-30       | cadence-override-retention | A hand-entered sprint cadence survives a disconnect and a project switch | done | ✅ Implemented 2026-08-31, six phases. The cadence stops being four columns on a row the Jira sync owns and becomes `sprint_cadence_override` — the `sprint_measurement` shape, keyed Jira-side, no FK into the sync graph, three independently nullable fields so **working days can be hand-set while length and start day still auto-pull** (a state FR-007 implied and no code path could reach). Row existence means "the lead has spoken for this sprint", NOT "the values differ" — copying `anomaly_settings`' rule would make *stop inheriting* inexpressible under a read-time inheritance tier (plan review F1). Three copy↔code contradictions closed: the restore was deleting the working days its own dialog promised to keep (pinned green by a test whose comment was false), every pre-action copy module returned zero hits for `cadence` while the values died with the credential, and the project-switch editor led to a disabled control. Two defects surfaced only by implementation — `saveCadence` compared against the very column it writes (the source is now derived), and the resolver's tier 1 was not project-scoped, re-creating S-26 impl-review F2's cross-workspace collision for a new payload. `carry` deleted with the unguarded `workingDays` hole it carried. `0023` additive, backfill a measured no-op. **Does NOT close S-32.** Original framing: Prereqs S-16, S-26 done. Found while framing S-26 (`context/changes/disconnect-data-retention/frame.md`) and deliberately left out of it: S-26 narrowed the cascade for `absence`, but `sprint` rows still die with the Jira credential in BOTH outcomes, and cadence carry-forward reads the *previous* sprint row (`reconcile-sprint.ts:190-231`). With that row gone the next reconcile reseeds from Jira's defaults with `cadenceOverridden: false`, so the lead's override is not lost loudly — it is replaced by a plausible wrong number. Unlike `absence`, this one cannot be fixed by a referential action: the values live ON the deleted row, so it needs the `sprint_measurement` treatment (a record that carries no FK into the sync lifecycle) or a cadence that belongs to the account rather than to a sprint. Overlaps S-29, which is where post-setup cadence UI now lives (that clause was split out of S-19 on the same day) |
...
| S-32       | cadence-single-home       | Retire what S-30 leaves behind: drop the superseded cadence columns    | done                   | ✅ Implemented, reviewed & merged — PR #102 (2026-08-31), three phases. Both leftovers closed. `0024_watery_rocket_racer.sql` drops `sprint.working_days` and `sprint.cadence_overridden`; the reader guard went in the same PR, one phase earlier because its own allowlist self-check fails the moment the writes it names are removed. Phase 1 removed every reader and writer **while the columns still existed**, so a green full suite was direct evidence the blast radius was the whole of it. Two blind spots the guard had were both WRITERS, not readers — the snake-case SQL of the `0023` backfill (its flag regex was camelCase) and `saveCadence`'s `update(sprint).set({ workingDays })`, a write by object key — which is why a schema constraint replaces the source scan rather than renewing it. **The retention half was closed by deciding NOT to prune**, on evidence rather than by deferral: the inheritance tier's lookback has no floor by construction, so pruning by age is the S-30 defect on a timer, and a record for a no-longer-monitored project is a promise the product makes to the lead in copy. Written into the PRD's retention non-goal and the table's own docblock, so the next reader finds the answer instead of the question. `0024` rides the next ordinary production migration run — the drop degrades cleanly if code gets there first, and production holds zero `sprint` rows |
...
- **DELIVERED 2026-08-30**, four phases on `feat/destructive-action-confirmation`.
  What shipped: one `DisconnectConfirmDialog` in front of all four paths, whose
  copy comes from `src/lib/integrations/disconnect-impact.ts` — a declaration a
  hermetic test holds equal to the schema's foreign-key graph, which is the part
  that outlives this slice. Six docstrings that stated a one-level cascade were
  corrected, `jira-project-editor.tsx`'s wrong warning was rebuilt from the same
  module (rooted at `sprint`, since a project switch updates `jira_project` in
  place and replaces the mappings), and the Connections tab was closed against
  demo: nine Server Actions now refuse server-side. **Not** in scope and still
  open: narrowing the cascade so absences survive (**S-26**), the structural demo
  boundary (**S-27**), and whether Disconnect should delete at all (**Open
  Roadmap Question 4**).
- **Raised by the tester, not by code review** (2026-08-30). The source note
  `context/manual-tests/S-16-4.6-brak-potwierdzenia-disconnect.md` was deleted
  together with the fix, per `CLAUDE.md` — a note that outlives its repair
  misleads the next reader — so the finding is recorded here instead. She stopped
...
### S-30: A hand-entered cadence survives a disconnect
...
- **✅ Implemented 2026-08-31, six phases.** The modelling question above is
  answered in the `sprint_measurement` shape: `sprint_cadence_override` is keyed
  by the JIRA-SIDE sprint id, its only foreign key points at the account, and its
  three fields are independently NULLABLE — where NULL means *follow the source
  for THIS field*. That last part is what makes the state FR-007 always implied
  reachable for the first time: **a Mon–Thu team whose length and start day still
  auto-pull from Jira.** One boolean over three columns could not express that
  account, so it described it wrongly.
  **Row existence means "the lead has spoken for this sprint", deliberately NOT
  `anomaly_settings`' "the value differs from the default"** (plan review F1):
  with a read-time inheritance tier, delete-when-equal makes *stop inheriting*
  inexpressible, and a lead saving their way back to the default would have it
  silently undone. So no write path deletes a row, and a three-NULL row is a
  meaningful state.
  **Three contradictions closed along the way**, none of them in the original
  outcome: "Restore Jira's values" was deleting the working-day pattern its own
  dialog promised to leave alone (Jira has no working-days field, so there was
  nothing to restore *from*) — pinned green by an integration test whose comment
  was false; every pre-action copy module returned **zero** hits for `cadence`
  while the values died with the credential; and the project-switch editor sent
  the lead to a screen whose restore button was disabled and whose save threw.
  **Two defects found only by writing the code.** `saveCadence` both wrote
  `sprint.length_days` and compared against it, so a second save of an unchanged
  override read as source-equal and dropped it — the source is now DERIVED
  (`deriveCadence` is pure and the row carries both dates, so it costs no network
  call). And the resolver's tier 1 keyed on `(owner, jira_sprint_id)` alone,
  which after a workspace re-point with a colliding sprint id handed the old
  team's cadence to the new sprint — the same collision S-26's impl-review F2
  found for `sprint_measurement`, now guarded by the same bridge.
  `carry` is deleted (inheritance is read-time), taking with it the unguarded
  hole it carried: its guard checked length and start day but **not** working
  days, then coalesced a NULL pattern to Mon–Fri on every rollover while writing
  `cadenceOverridden: true`. Migration `0023` is additive with a measured-no-op
  backfill. **Does NOT close S-32** — the superseded columns stay, held by a
  hermetic reader guard whose allowlist is the contract.
...
- **Two deliberate leftovers, from the same decision.** S-30 moves the cadence
  ...
  S-30's Phase 5 (`src/lib/cadence-override-readers.test.ts`), whose allowlist IS
  the contract. That is a fine bridge and a bad permanent arrangement: a guard can
  be deleted by anyone who reads its failure as noise, and the question "which
  copy is true" comes straight back. **Read its stated blind spot before relying
...
- **S-30: the sprint cadence a lead entered by hand — length, start day, working days — is still theirs after a Jira disconnect or a project switch, instead of being replaced by Jira's defaults on the next reconcile.** — Archived 2026-08-31 → `context/archive/2026-08-31-cadence-override-retention/`. Lesson: —. Note: the three copy↔code contradictions this slice closed were not the end of them. An impl-review after the epilogue found the same shape three more times — a path asserting, on the lead's behalf, something the lead never said: a restore materialising Mon–Fri as *their* choice on an account that had never picked one, two modules promising a reattachment the record's Jira-side keying makes impossible, and the fallback diagnostic reporting a deliberate project switch as a failure on every cycle forever. All three fixed before archive (`7d7270c`, `19e1bb1`); a fourth finding was a test that could not fail. **Does NOT close S-32** — the two superseded columns stay, held by one hermetic reader guard.
- **S-32: the cadence a lead chose has exactly one home in the database — the superseded copy on `sprint` is dropped, the reader guard holding the line goes with it, and the retention question is answered: nothing prunes the record, on purpose.** — Archived 2026-08-31 → `context/archive/2026-08-31-cadence-single-home/`. Lesson: —. Note: the retention half was closed by deciding NOT to prune, on evidence rather than by deferral — the inheritance tier's lookback has no floor by construction, so pruning by age is the S-30 defect on a timer, and a record for a no-longer-monitored project is a promise the product makes to the lead in copy. Written into the PRD's retention non-goal and the table's own docblock so the next reader finds the answer instead of the question. The reader guard's two blind spots both turned out to be WRITERS, not readers, which is why a schema constraint replaces the source scan rather than renewing it. `0024` still owes production a run (backlog 27.A); the drop degrades cleanly if code gets there first, but applying it spends S-30's escape hatch — a revert to any pre-S-32 commit then fails 42703 on every reconcile.
- **S-17: the team's public holidays are DERIVED from a country stored on the account, and re-offered every year as a proposal the lead approves — instead of being typed in date by date, or not at all.** — Archived 2026-08-31 → `context/archive/2026-08-31-working-days-calendar/`. Lesson: —. Note: **the Outcome line above is pre-S-23 and was already true when this slice opened** — S-23 wired `getNonWorkingDays` into all five elapsed-time rules and the capacity divisor, so a recorded holiday already stopped the clock. What was NOT true is that anybody had recorded one: measured 2026-08-31, **no real account held a single `team_day_off` row**, so the mechanism ran against an empty calendar and said nothing about it. Hence three things, of which the derivation is the smallest — the sentence, the records, the rules, the proposal. `holiday_year_approval` rather than `team_day_off.source` is what keeps a deleted holiday deleted: a deleted row leaves no provenance behind to consult, so only a record saying *this year has been decided about* stops the next render re-offering it. Phase 1's empty-calendar notice is deliberately RETIRED in Phase 4 for the same reason — the approval record, not the row count, is what says the calendar was reviewed, and a team that unchecked every holiday because it works them verified the opposite of what that sentence claimed. `0025` was applied to production the same day (backlog 28.0) and is reversible, unlike `0024`. Impl-review found the slice's own annual-regression argument reproduced one level up: the proposal window followed `getActiveSprintRow`, which returns the most-recently-STARTED sprint when none is active, so between sprints the surfaces asked about a closed sprint's year and went silent on the year the team was living in (F1, fixed); the server also took the client's word for WHICH years were being approved (F3, fixed). Eleven manual rows open in `manual-test-backlog.md` §28.
- **S-18: the availability tab answers "can I promise this?" with a NUMBER for the next window, not only with who is away.** — Archived 2026-09-01 → `context/archive/2026-08-31-next-sprint-capacity/`. Lesson: —. Note: the number was the smallest part. **Two of its three inputs degraded silently past the active sprint's end, both in the same direction**, so the figure was systematically too high with no offsetting term — hence the slice ships the number, the two closures, and the sentence saying what it may claim, in that order. The window itself stopped being an extrapolation of the CURRENT sprint's accidental millisecond span and became the lead's resolved cadence (S-29/S-30/S-32 made the old rationale comment — "those columns are read by nothing" — false), which moved the derivation server-side; **every account's "Next window" grid therefore loses one column**, because a real Jira sprint ends at the same time of day it starts and the old rule drew `lengthDays + 1`. The holiday-review horizon was extended one window right and, more importantly, given ONE spelling for all three surfaces: `approveHolidayYearAction` re-derives it server-side and refuses anything outside, so a page offering 2027 against a validator ending in 2026 was a button that could never succeed. **S-18's original scope is NOT closed** — projecting an UNSTARTED window (its own working-day config, its own absence coverage, neither exposed by Jira before the sprint exists) remains out; what shipped is the forecast window the tab already drew, now measured and labelled. Impl-review found two defects of the same family the slice exists to fight, both fixed before archive (`b05ab65`): `hasForwardAbsence` compared raw instants where `absence.end_date` is a day's LAST instant and `sprint.end_date` is Jira's arbitrary one, so an absence on the sprint's own final day read as "forward" and silenced the notice for exactly the lead it was written for (reproduced against Postgres, not inferred); and the absence read's bound claimed an invariant it did not hold at the 90-day ceiling, where a DST fall-back consumed a zero margin and would have dropped the window's last day silently. Five manual rows open in `manual-test-backlog.md` §29 — 29.C is the one that proves the widened bound actually moves the number.
