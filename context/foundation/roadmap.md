...
| S-32       | —                         | Retire what S-30 leaves behind: drop the superseded cadence columns    | yes                    | Prereq S-30 done (archived 2026-08-31). ⚠️ **This row was missing until 2026-08-31** — S-32 had a full detail block but appeared in neither summary table, while `src/db/schema.ts` and `src/lib/cadence-override-readers.test.ts` both send the reader here as the place those columns get dropped. A slice invisible in the tables is a dangling promise. Two halves with different characters: the migration is a chore, the retention decision is not — an override for a long-closed sprint is exactly what tier-2 inheritance reads to carry a Mon–Thu pattern forward, so a prune that is too eager rebuilds the S-30 defect inside a cron job |
