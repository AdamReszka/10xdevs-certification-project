...
| S-09       | demo-mode                 | Demo mode: load/reset mixed-state fixture dataset                      | no                     | Prereqs S-07, S-10 done. **Unblocked 2026-08-29** — Open Question #1 answered (any account may hold both) and the frame found it was not the blocker: the slice needs a demo/real distinction the schema and the reconciling writers both respect. Two plan-bearing findings — `db:seed:demo` clears 14 tables incl. both credential tables, which the chosen scope makes destructive; and `detectAnomalies` RESOLVES every seeded anomaly it does not re-derive, reachable even by saving an absence. See `context/changes/demo-mode/frame.md` |
