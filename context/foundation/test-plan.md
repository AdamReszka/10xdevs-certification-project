...
| (optional) AI-native | LLM-as-judge eval for FR-020 Refinement Helper — checked: 2026-06-16; **re-scoped 2026-08-26** | n/a | **No longer on the path to FR-020.** The original entry created a circular dependency — the eval waited for S-13 to ship, while S-13's load-bearing requirement waited for the eval. S-13's framing (`context/changes/refinement-helper-ai/frame.md`) dissolved it: the output is a *gap statement grounded in the ticket's own content*, so grounding is the required sentence shape rather than a style property needing a judge, and "did it flag gap class N on this ticket" is an ordinary deterministic assertion over a corpus of tickets with known gaps — including complete tickets whose only correct verdict is "DOR met". A judge remains optional for grading gap *wording quality*, which nothing depends on. |
