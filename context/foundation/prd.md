...
- FR-024: From the per-sprint history (FR-023) the system computes an **estimated velocity for the next sprint**: the average of past sprints' normalised velocity, scaled by the ratio of the next sprint's capacity to its full capacity. Worked example: ten full-time people over 20 working days is 200 MD; one person away for the whole sprint makes it 180 MD, a 10% reduction, so an average of 100 SP yields an estimate of 90 SP. The estimate is shown together with the capacity and velocity figures it was derived from, is presented as a suggestion the lead may ignore, and is withheld entirely when there is no history to average. Priority: must-have
