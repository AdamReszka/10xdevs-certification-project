...
- FR-007: System pulls sprint cadence (length, start day, working days) from the monitored Jira project's active-sprint configuration on each sync; user can override the auto-pulled values when their Jira sprint config diverges from their actual cadence; user can additionally record days on which the WHOLE team is off — public holidays, company days off — as dates on the account, so one entry applies to every sprint that spans it. Priority: must-have
  ...
  > Socratic (revised 2026-08-31 — `context/changes/cadence-override-retention/`): the override's home is now stated, and the change is not cosmetic. It used to be four columns on the row the Jira sync owns — a row that dies with the Jira credential in BOTH of S-26's disconnect outcomes and is deleted outright on a project switch. The next reconcile then reseeded from Jira's configuration and recorded that the lead had chosen nothing: **the override was not lost loudly, it was replaced by a plausible wrong number**, on the one field the lead entered precisely because Jira's answer was wrong for their team. It now lives in a **record keyed by the Jira-side sprint id with no foreign key into the sync graph** (the `sprint_measurement` shape), so it outlives the credential, the project switch and the rollover alike. Two consequences belong in the requirement rather than in the code. **The three values are protected independently**, because their provenance genuinely differs: length and start day are derived from Jira's dates and are what "auto-pull" means, while **working days have no upstream in Jira at all** — the field does not exist there — so they are lead-owned, and their protection cannot be governed by the flag that governs the other two. That is what makes a Mon–Thu team with auto-pulled length and start day expressible for the first time; the sentence above always implied that account, and no code path could reach it. **And "restore the auto-pulled values" means the two Jira supplies, never the third**: restoring from Jira a field Jira does not have is not a restore, it is deleting the lead's choice under someone else's name.
...
- FR-009: System ships with sensible default thresholds for every anomaly rule. **Every elapsed-time threshold is denominated in working time**: the clock advances only between 08:00 and 16:00 in the team's time zone, only on days in the sprint's configured working days (FR-007), and never on a day recorded as a team-wide day off (FR-007); an individual team member's absence (FR-010) does NOT pause it. Eight working hours make one working day. The shipped defaults are: PR-review timeout 8 working hours; ticket-in-status timeouts with per-story-point variants for In-Progress — 1/2 SP=8, 3 SP=16, 5 SP=24, 8/13 SP=40, 21 SP=64 working hours — plus Code Review=8 and Testing=16 working hours; max-parallel limits for In-Progress / Code Review / Testing per developer; PR size limit; scope-creep percentage; a no-commit window in working days; and a ToDo-before-sprint-end alert lead time of 16 working hours. Threshold tuning is NOT part of the initial setup wizard — it lives on a dedicated settings page the user reaches after first run. Priority: must-have
  ...
  > Socratic (revised 2026-08-30 — `context/changes/working-day-aging/`): the original list mixed three units in one sentence ("1/2 SP=24h … 8/13 SP=5 days … 21 SP=8 working days") and never said how a weekend was treated, so the requirement could be read as wall-clock or as working-day and the code read it both ways — three of roughly nine elapsed-time measurements skipped weekends, the rest did not. **The mixed units were never a decision.** The archive holds no argument for wall-clock aging; the only recorded working-day discussion is the 21-SP sentinel, which existed precisely because "8 working days" was inexpressible in a field denominated in wall-clock hours. Stating the unit once, at the top, retires that sentinel: 8 working days is 64 working hours, an ordinary number. The numbers change because the unit does — each budget keeps the intent it shipped with (24h meant "a day", and a day is 8 working hours), so a lead who never touched the settings page gets the same promise honoured against a calendar that matches their team's. **The 08:00–16:00 window is a deliberate hard-coded average, not a config surface**: Jira exposes no working-hours field (FR-007's own note), a per-team window would need a column, a migration and a settings page, and at these budget sizes an hour either side of a person's real start time cannot change which day an anomaly lands on. **Individual absences deliberately do not stop the clock**: the sprint is the team's, and the inbox is an alert for the lead about work that is not moving, not a device pointed at a person — which is also what the no-per-developer-framing guardrail requires. FR-010's separate promise stands untouched: an absence still suppresses `DEVELOPER_INACTIVE` outright for that person. FR-013 and FR-016 need no amendment — neither states a unit.
...
- **No retention of raw synced data beyond current + 2 previous sprints.** Tickets, pull requests, commits, status history and daily recaps are bounded to that window. **Exception (2026-08-27, FR-023):** the per-sprint *measurement record* — capacity, adjusted capacity and delivered story points, a few dozen bytes per sprint — is retained for the team's whole lifetime, because an average that resets every three sprints is not an average. This is a deliberate amendment, not a workaround: retaining the record is not the same as building a surface for it, so inter-sprint **trend dashboards**, multi-quarter history and year-over-year analytics remain explicit phase-2. **Clarified 2026-08-28:** looking at ONE closed sprint's own figures — a sprint switcher on Sprint Detail — is not a trend dashboard and is in scope; what stays parked is any surface that plots a series across sprints. **Extended 2026-08-31 (S-32):** the same exception covers the per-sprint *cadence-override record* (FR-007) — the lead's chosen length, start day and working days, a handful of bytes per sprint — for the same reason, one step further on: an inheritance chain that resets every three sprints is not inheritance. The resolver's inheritance tier orders on `start_date <= <this sprint's>` with no floor by construction, so the record that carries a Mon–Thu pattern into today's sprint is routinely one from a long-closed sprint, and pruning by age would silently hand the team back Mon–Fri — the S-30 defect rebuilt on a timer. **Nothing deletes such a record, ever, and that is a decision rather than an omission:** a record for a project the account no longer monitors is also a promise the product makes to the lead in copy, that a project switch keeps the cadence they set by hand.
